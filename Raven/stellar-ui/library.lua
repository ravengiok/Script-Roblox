...
function Update:Window(Config)
	...
	MinimizeButton.MouseButton1Click:connect(function()
		...
		if isMinimized then
			...
			local minimizedHeight = 55;
			OutlineMain:TweenSize(UDim2.new(0, originalOutlineSize.X.Offset, 0, minimizedHeight), "Out", "Quad", 0.3, true);
			Main:TweenSize(UDim2.new(1, 0, 0, 40), "Out", "Quad", 0.3, true);
			...
			OutlineMain:TweenSize(UDim2.new(0, originalOutlineSize.X.Offset, 0, originalOutlineSize.Y.Offset), "Out", "Quad", 0.3, true);
			Main:TweenSize(UDim2.new(0, originalSize.X.Offset, 0, originalSize.Y.Offset), "Out", "Quad", 0.3, true);
	...
	local function CreateDropdown(title, options, default, callback)
		...
		local function toggleDropdown()
			...
			if isOpen then
				...
				DropdownFrame:TweenSize(UDim2.new(0.4, 0, 0, math.min((#options) * 30, 150)), "Out", "Quad", 0.3, true);
			else
				DropdownFrame:TweenSize(UDim2.new(0.4, 0, 0, 0), "Out", "Quad", 0.3, true, function()
					DropdownFrame.Visible = false;
				end);
			end;
		end;
	...
	local themeDropdown = CreateDropdown("Theme", {
		"Default",
		"Blue",
		"Purple",
		"Green",
	...
	function uitab:Tab(text, img)
		...
		function main:Dropdown(text, options, default, callback)
			...
			local function toggleDropdown()
				...
				if isOpen then
					...
					OptionsFrame:TweenSize(UDim2.new(1, 0, 0, math.min((#options) * 32, 160)), "Out", "Quad", 0.3, true);
					(TweenService:Create(Arrow, TweenInfo.new(0.3), {
						Rotation = 180
					})):Play();
				else
					OptionsFrame:TweenSize(UDim2.new(1, 0, 0, 0), "Out", "Quad", 0.3, true, function()
						OptionsFrame.Visible = false;
					end);
					(TweenService:Create(Arrow, TweenInfo.new(0.3), {
						Rotation = 0
		...
		function Update:WindowWithNestedTabs(config)
			local window = Update:Window(config);
			local originalTabFunc = window.Tab;
			function window:TabWithSubs(name, icon)
				local mainTab = originalTabFunc(name, icon);
				local extendedAPI = {};
				for funcName, func in pairs(mainTab) do
					extendedAPI[funcName] = func;
				end;
				function extendedAPI:SubTab(subName, subIcon)
					local SubTabButton = Instance.new("TextButton");
					SubTabButton.Parent = mainTab:GetPage();
					SubTabButton.Name = "SubTab_" .. subName;
					SubTabButton.Text = "";
					SubTabButton.BackgroundColor3 = _G.Primary;
					SubTabButton.BackgroundTransparency = 0.9;
					SubTabButton.Size = UDim2.new(1, -30, 0, 30);
					SubTabButton.Position = UDim2.new(0, 15, 0, 0);
					CreateRounded(SubTabButton, 4);
					local SubContent = Instance.new("Frame");
					SubContent.Name = "SubContent_" .. subName;
					SubContent.Parent = SubTabButton;
					SubContent.BackgroundTransparency = 1;
					SubContent.Position = UDim2.new(0, 20, 0, 35);
					SubContent.Size = UDim2.new(1, -20, 0, 0);
					SubContent.ClipsDescendants = true;
					local SubList = Instance.new("UIListLayout");
					SubList.Parent = SubContent;
					SubList.SortOrder = Enum.SortOrder.LayoutOrder;
					SubList.Padding = UDim.new(0, 5);
					local Label = Instance.new("TextLabel");
					Label.Parent = SubTabButton;
					Label.Text = "  " .. subName;
					Label.Font = Enum.Font.Gotham;
					Label.TextSize = 13;
					Label.TextColor3 = _G.Themes[_G.CurrentTheme].Text;
					Label.Position = UDim2.new(0, 30, 0, 5);
					Label.Size = UDim2.new(0.8, 0, 0, 20);
					Label.TextXAlignment = Enum.TextXAlignment.Left;
					local IconImg = Instance.new("ImageLabel");
					IconImg.Parent = SubTabButton;
					IconImg.Image = subIcon or "rbxassetid://10734904596";
					IconImg.Size = UDim2.new(0, 15, 0, 15);
					IconImg.Position = UDim2.new(0, 10, 0, 8);
					IconImg.ImageColor3 = _G.Themes[_G.CurrentTheme].Text;
					local Arrow = Instance.new("ImageLabel");
					Arrow.Parent = SubTabButton;
					Arrow.Image = "rbxassetid://10709790948";
					Arrow.Size = UDim2.new(0, 12, 0, 12);
					Arrow.Position = UDim2.new(1, -25, 0, 9);
					Arrow.ImageColor3 = _G.Themes[_G.CurrentTheme].Text;
					local isOpen = false;
					SubTabButton.MouseButton1Click:Connect(function()
						isOpen = not isOpen;
						if isOpen then
							SubContent:TweenSize(UDim2.new(1, -20, 0, SubList.AbsoluteContentSize.Y), "Out", "Quad", 0.3, true);
							SubTabButton:TweenSize(UDim2.new(1, -30, 0, 35 + SubList.AbsoluteContentSize.Y), "Out", "Quad", 0.3, true);
							Arrow.Rotation = 0;
							SubTabButton.BackgroundTransparency = 0.7;
						else
							SubContent:TweenSize(UDim2.new(1, -20, 0, 0), "Out", "Quad", 0.3, true);
							SubTabButton:TweenSize(UDim2.new(1, -30, 0, 30), "Out", "Quad", 0.3, true);
							Arrow.Rotation = -90;
							SubTabButton.BackgroundTransparency = 0.9;
						end;
					end);
					local subAPI = {};
					function subAPI:Button(text, callback)
						local btn = mainTab:Button(text, callback);
						btn.Parent = SubContent;
						return btn;
					end;
					function subAPI:Toggle(text, default, desc, callback)
						local toggle = mainTab:Toggle(text, default, desc, callback);
						toggle.Parent = SubContent;
						return toggle;
					end;
					function subAPI:Slider(text, min, max, default, callback)
						local slider = mainTab:Slider(text, min, max, default, callback);
						slider.Parent = SubContent;
						return slider;
					end;
					function subAPI:Dropdown(text, options, default, callback)
						local dropdown = mainTab:Dropdown(text, options, default, callback);
						dropdown.Parent = SubContent;
						return dropdown;
					end;
					function subAPI:Textbox(text, placeholder, callback)
						local textbox = mainTab:Textbox(text, placeholder, callback);
						textbox.Parent = SubContent;
						return textbox;
					end;
					return subAPI;
				end;
